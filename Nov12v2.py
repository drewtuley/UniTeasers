...
def test_words_against_dictionary(words, valid_words, dictionary):
    invalid_words = []
...
if __name__ == '__main__':
    with open('20k.txt') as fd:
        dictionary = set(filter(lambda word: (len(word) == 4), [x.strip() for x in fd]))

        test = test_words_against_dictionary(['snap', 'nape', 'apes', 'pest'], [], dictionary)
        if len(test) == 0:
        ...
        for p in permutations(['a', 'e', 'n', 'p', 'p', 's']):
            ...
            if not any(word in invalid_word_cache for word in words_to_test):
                bad_words = test_words_against_dictionary(words_to_test, valid_word_cache, dictionary)
                if len(bad_words) == 0:
                    print(words_to_test)
                    break
                else:
